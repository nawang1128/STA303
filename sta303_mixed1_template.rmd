...
author: 'Chief Science Officer Ruotong Wang; ID: 1004235712'
output:
  html_document:
    df_print: paged
...
- \usepackage{fancyhdr}
- \pagestyle{fancy}
- \fancyhead[LO, LE]{Ruotong Wang, 1004235712}
...
```{r}
co_officer <- crew_data %>% filter(position == "Communications Officer")
unique(co_officer$name)
```

3. How many crewmembers are in this dataset? 

```{r}
members <- crew_data %>% distinct(name)
count(members)
```
...
en_sub <- crew_data %>% filter(sub_division == "Engineering")
engineer <- en_sub %>%
  filter(str_detect(position, "Engineer"))
engineering <- en_sub %>%
  filter(str_detect(position, "Engineering"))
prop_engineer <- count(engineer) / count(en_sub)
prop_engineer
prop_engineering <- count(engineering) / count(en_sub)
prop_engineering
```
...
model_3 <- lmer(productivity ~ week + starfleet_gpa + perseverance_score +
                  (1 + week|name) + + (1|duty_shift:shift_team) + (1 + week|full_team),
                data = crew_data)
...
complete_comp <- merge(x = complete_comp, y = origin_data, by = "plant", all.y = TRUE)
complete_comp <- na.omit(complete_comp) %>%
  distinct(crewmember, native_to)

Alexis <- complete_comp %>% filter(crewmember == "Alexis Alarid (42017)")
count(Alexis)
Bailey <- complete_comp %>% filter(crewmember == "Bailey Ellison (42034)")
count(Bailey)
Jacky <- complete_comp %>% filter(crewmember == "Jacky Padua (42096)")
count(Jacky)
```
...
library(reprex)
origin_data %>% 
  filter(native_to == "Delta Quadrant")
reprex()

# The words show on the clipboard
